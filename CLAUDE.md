...
Claude + Apify                bookmyshow (403, needs browser + residential proxy)
...
data/raw/<date>__<source>.json      ← Claude writes these (the only network step)
...
- **stdlib only.** No pandas, no requests, no openpyxl. Deliberate: the compute half must run
  anywhere, and the network half is Claude's job, not Python's.
